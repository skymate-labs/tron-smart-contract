...
contract SkymateNft is TRC721 {
    ...
    // address admin -> eventId -> true/false
    mapping(address => mapping(uint8 => bool)) private _adminEvents;

    // address checker -> eventId -> true/false
    mapping(address => mapping(uint8 => bool)) private _ticketChecker;

    // eventId -> address checker
    mapping(uint8 => address[]) private _listTicketChecker;
    ...
    function setTicketChecker(
        ...
        if(_ticketChecker[ticketChecker_][eventId_]) {
            _listTicketChecker[eventId_].push(ticketChecker_);
        }
    }
    ...
    function getListTicketChecker(uint8 eventId_)
        public
        view
        returns (
            address[] memory
    ...
    {
        address[] memory reponseListTicketChecker = new address[](_listTicketChecker[eventId_].length);
        for (uint256 index_ = 0; index_ < _listTicketChecker[eventId_].length; index_++) {
            reponseListTicketChecker[index_] = _listTicketChecker[eventId_][index_];
        }
        return reponseListTicketChecker;
    }
